...
def parse_model(d, ch, verbose=True):  # model_dict, input_channels(3)
    ...
    dynamic_imports = {  # 在模块解析循环前添加动态导入字典
        'SEAttention': ('ultralytics.nn.modules.attention', 'SEAttention'),
        'CBAM': ('ultralytics.nn.modules.attention', 'CBAM')
    }
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        # 动态处理自定义模块 ------------------------------------------------------
        original_m = m  # 保留原始模块名用于错误提示
        if isinstance(m, str):  # 确保处理的是字符串类型的模块名
            if m in dynamic_imports:
                module_path, module_name = dynamic_imports[m]
                try:
                    import importlib
                    mod = importlib.import_module(module_path)
                    m = getattr(mod, module_name)
                except (ImportError, AttributeError) as e:
                    raise ImportError(f"Failed to import {m} from {module_path}: {e}")
            else:
                # 处理内置模块
                try:
                    m = (
                        getattr(torch.nn, m[3:])  # 添加闭合括号
                        if "nn." in m
                        else getattr(__import__("torchvision").ops, m[16:])
                        if "torchvision.ops." in m
                        else globals().get(m, None)  # 正确闭合所有括号
                    )
                except Exception as e:
                    LOGGER.error(f"Error parsing module {m}: {e}")
                    raise
        else:
            m = m  # 如果 m 已经是模块类（如直接传入的 nn.Module）

        # 如果模块仍未找到，抛出明确错误
        if m is None:
            raise ImportError(f"Module '{original_m}' not found. Check if it's registered in dynamic_imports or globals()")
